...
| `+b ~Z:…` | There is no such type, and a mask naming one is refused with 696 rather than kept. An extended ban whose type this server does not have falls through to being matched as a plain glob — a ban on somebody literally called `~Z:spammer`, which is nobody — and it looks exactly like a ban that works. The same goes for a type given nothing to work with (`~a:`), `~O` given something, `~j:` given a name that is not a channel, and a mask wrapped in itself past any sense. Exceptions and invite exceptions are checked the same way: one that matches nobody is worse than a ban that does |
