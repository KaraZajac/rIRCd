...
# A second plain listener: connection ids used to come from a counter per
# listener, so two ports handed the same id to two different people.
IRC_PORT2="${SMOKE_IRC_PORT2:-16669}"
WS_PORT="${SMOKE_WS_PORT:-16668}"
# A TLS client port too, with a throwaway certificate, so what only a real
...
if [ ! -f "$ETC_DIR/tls.cert.pem" ]; then
  openssl req -x509 -newkey rsa:2048 -nodes -days 2 \
    -keyout "$ETC_DIR/tls.key.pem" -out "$ETC_DIR/tls.cert.pem" \
    -subj "/CN=irc.smoke.test" >/dev/null 2>&1 || { echo "could not make a TLS certificate" >&2; exit 1; }
fi
...
[server]
name = "irc.smoke.test"
listen = ["$BIND:$IRC_PORT", "$BIND:$IRC_PORT2"]
listen_ws = ["$BIND:$WS_PORT"]
listen_tls = ["$BIND:$TLS_PORT"]
motd = """
...
export SMOKE_IRC_PORT2="$IRC_PORT2"
export SMOKE_DB_SOCKET="$DB_SOCK"
export SMOKE_MAIL_DIR="$MAIL_DIR"
export SMOKE_UPLOAD_DIR="$RUN_DIR/uploads"
